...
app.get('/end', isAuthenticated, async function (req, res) {
    ...
    const user = await collection.findOne({ idNumber: "test2" })
    client.close()
...
app.get('/Results/:exam', isAuthenticated, async function (req, res) {
    ...
    const user = await collection.findOne({ idNumber: "test2" })
    if (user[req.params.exam]) {
        res.render(`${req.params.exam}Result`, { [req.params.exam]: JSON.stringify(user[req.params.exam]), name: "test2", from: "show" });
        return;
    }
    res.send('notFound');
})
...
app.post('/saveExam', async function (req, res) {
    ...
    await collection.updateOne({ idNumber: "test2" }, { $set: object }).then(() => {
...
app.post('/deleteExam', async function (req, res) {
    ...
    const collection = db.collection('users');
    // console.log("test2")
    await collection.updateOne({ idNumber: "test2" }, {
        $unset: {
            [req.body.type]: "",
            [`stat.${req.body.type}`]: ""
        }
...
app.post('/deleteStudent', async function (req, res) {
    ...
    const collection = db.collection('users');
    // console.log("test2")
    await collection.deleteOne({ idNumber: "test2" }).then(() => {
        res.send("deleted")
...
app.post('/login', express.urlencoded({ extended: false }), async function (req, res) {
    ...
    // {projection:{ email: 1, }}
    const user = await collection.findOne({ user: "test2", pass: "123456" })
    if (user) {
