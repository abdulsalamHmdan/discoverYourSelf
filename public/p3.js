...
function showQuestion(index) {
    ...
    setTimeout(() => {
        ...
        document.getElementById('currentIntelligenceName').textContent = 'اختبار الذكاءات المتعددة';
        ...
        document.getElementById('progressText').textContent =
            `تمت الإجابة على ${answeredCount} من ${questions.length} · المتبقي: ${questions.length - answeredCount} سؤالًا`;
...
function calculateResults() {
    ...
    Object.keys(intelligences).forEach(type => {
        ...
        diagnostics.byIntelligenceType[type] = {
            ...
            performanceLevel: scoreDifference > 15 ? 'ممتاز' :
                scoreDifference > 5 ? 'جيد جداً' :
                    scoreDifference > -5 ? 'متوسط' :
                        scoreDifference > -15 ? 'أقل من المتوسط' : 'يحتاج تطوير'
        };
    });
    ...
    Object.keys(scores).forEach(type => {
        ...
        if (typeData.completionRate < 50) {
            diagnostics.validation.warnings.push(`نسبة إكمال منخفضة للذكاء ${intelligences[type].name}: ${typeData.completionRate}%`);
        }

        // تحقق من منطقية النتائج
        if (scores[type] === 0 && typeData.answeredQuestions > 0) {
            diagnostics.validation.errors.push(`نتيجة صفر غير منطقية للذكاء ${intelligences[type].name} رغم وجود إجابات`);
        }

        if (scores[type] > 95 && typeData.completionRate === 100) {
            diagnostics.validation.warnings.push(`نتيجة عالية جداً للذكاء ${intelligences[type].name}: ${scores[type]}%`);
        }
    });
    ...
    if (diagnostics.statistics.mean) {
        if (diagnostics.statistics.mean < 15) {
            diagnostics.validation.warnings.push(`متوسط النتائج منخفض جداً: ${diagnostics.statistics.mean}%`);
            diagnostics.validation.isReliable = false;
        }

        if (diagnostics.statistics.mean > 85) {
            diagnostics.validation.warnings.push(`متوسط النتائج مرتفع جداً: ${diagnostics.statistics.mean}%`);
        }

        if (diagnostics.statistics.standardDeviation < 3) {
            diagnostics.validation.warnings.push(`تشابه كبير في النتائج قد يشير لعدم دقة الإجابات`);
        }
    }
    ...
    console.log('✅ موثوقية النتائج:', diagnostics.validation.isReliable ? 'عالية' : 'منخفضة');
    console.groupEnd();
...
function proceedWithResults() {
    ...
    if (!isReliable) {
        console.warn('⚠️ تحذير: النتائج قد تكون غير دقيقة بسبب عدم اكتمال الإجابات');
...
function exportDataToCSV() {
    ...
    link.setAttribute('download', `بيانات_الذكاءات_المتعددة_${new Date().getTime()}.csv`);
    ...
    printWindow.document.write(`
                ...
                <body>
                    <div class="header">
                        ...
                        <p><strong>وقت الطباعة:</strong> ${new Date().toLocaleString('ar-SA')}</p>
                    </div>
                    ...
                    <div class="stats">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 10px;">
                            ...
                            <div><strong>الأسئلة غير المجابة:</strong> ${totalQuestions - answeredQuestions} سؤال</div>
                            ...
                            <div class="intelligence-section">
                                <div class="intelligence-header">
                                    ...
                                    <div class="type-stats">
                                        <span>الأسئلة: ${questionsOfType.length}</span>
                                ...
                                ${questionsOfType.map(item => `
                                    <div class="question-item ${item.isAnswered ? 'answered' : 'unanswered'}">
                                        <div class="question-number">السؤال رقم ${item.questionNumber}</div>
                                        <div class="question-type">
                                            ${item.questionType === 'single' ? 'اختيار واحد' :
                item.questionType === 'multiple' ? 'اختيار متعدد' :
                    item.questionType === 'ranking' ? 'ترتيب' :
                        item.questionType === 'rating' ? 'تقييم بالنجوم' : item.questionType}
                                        </div>
                    ...
                    <div class="summary-box page-break">
                        ...
                        ${answeredQuestions < totalQuestions ? `
                            <p style="color: #dc2626;">
                                <strong>تنبيه:</strong> لم يتم الإجابة على ${totalQuestions - answeredQuestions} أسئلة. 
    ...
    printWindow.document.write(`
                ...
                <body>
                    <div class="header">
                        ...
                        <p><strong>اسم المستخدم:</strong> ${currentUser}</p>
                        <p><strong>تاريخ الاختبار:</strong> ${new Date().toLocaleDateString('ar-SA')}</p>
                        <p><strong>وقت الاختبار:</strong> ${new Date().toLocaleTimeString('ar-SA')}</p>
                    </div>
                    ...
                    ${sortedIntelligences.map((intel, index) => `
                        <div class="intelligence-item ${index < 3 ? 'top-three' : ''}">
                            ...
                            ${index < 3 ? '<p><em>⭐ من أقوى ذكاءاتك</em></p>' : ''}
                        </div>
                    `).join('')}
